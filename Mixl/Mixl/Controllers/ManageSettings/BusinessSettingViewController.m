//
//  BusinessSettingViewController.m
//  Mixl
//
...
#import "BusinessSettingViewController.h"

@interface BusinessSettingViewController ()
{
    int searchRadius;
    int ageMin;
    int ageMax;
    ...
    NSString* whoSee;
    NSString* whoContact;
    NSString* pageVisibility;
    NSString* receiveMessage;

}
@end

@implementation BusinessSettingViewController
...
- (void) initData{
    ...
    pageVisibility = [userSettings objectForKey:@"page_visibility"];
    receiveMessage = [userSettings objectForKey:@"accept_messages"];
...
- (void) initView {
    ...
    if ([pageVisibility isEqualToString:@"1"]) {
    ...
    if ([receiveMessage isEqualToString:@"1"]) {
...
- (void)onSaveSettings {
    ...
    [dic setObject:pageVisibility forKey:@"page_visibility"];
    [dic setObject:receiveMessage forKey:@"accept_messages"];
...
- (void) requestUpdateSettings:(id) params {
    ...
    if (resObj != nil) {
        ...
        if([status intValue] == 0) {
            ...
            NSLog(@"---------Venue info updated settings : %@", appController.currentUser);
...
- (void) Done{
    ...
    (PeopleNearbyViewController*) [storyboard instantiateViewControllerWithIdentifier:@"PeopleNearbyVC"];
    [self.navigationController pushViewController:peopleNearbyViewController animated:YES];
}
