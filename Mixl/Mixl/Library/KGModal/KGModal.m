...
- (void)showWithContentViewc:(WFIntroModalViewController *)v andAnimated:(BOOL)animated byContent:(NSString *)content {
    self.window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    self.window.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
    self.window.opaque = NO;
    ...
    CGFloat padding = 13;
    CGRect containerViewRect = CGRectInset(v.view.bounds, -padding, -padding);
    containerViewRect.origin.x = containerViewRect.origin.y = 0;
    containerViewRect.origin.x = round(CGRectGetMidX(self.window.bounds)-CGRectGetMidX(containerViewRect));
    containerViewRect.origin.y = round(CGRectGetMidY(self.window.bounds)-CGRectGetMidY(containerViewRect));
    KGModalContainerView *containerView = [[KGModalContainerView alloc] initWithFrame:containerViewRect];
    ...
    UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleBottomMargin;
    containerView.layer.rasterizationScale = [[UIScreen mainScreen] scale];
    v.view.frame = (CGRect){padding, padding, v.view.bounds.size};
    [v setIntroContent:content];
    [containerView addSubview:v.view];
    [viewController addChildViewController:v];
    [viewController.view addSubview:containerView];
    self.containerView = containerView;
...
//    styleLayer.borderColor = [ORANGECOLOR CGColor];
